#!/usr/bin/env python3
"""
USMA (Unified Screen Monitoring Application) - v.0.3.3
...
v.0.3.3 Changes:
- Enhanced live feedback panel to show average HF Energy Ratio and HF Energy.
- Redesigned the summary image log into a comprehensive analysis collage, showing
  plots and data for every monitored region in one image.
- Added high frequency energy value to the debug logs.
- Fixed an issue where the screen overlay was captured in screenshots,
  contaminating the analysis. The overlay is now drawn with a padded offset.
"""
...
logger.setLevel(logging.DEBUG) # Set to logging.DEBUG to see detailed logs
...
class ScreenMonitor:
    ...
    def start(self, verbose_logging=True, image_logging=True):
        ...
        self.thread.start()
        logger.info(f"Screen monitoring thread started for USMA v.0.3.3")
        return True
    ...
    def _validate_signal_quality(self, color_mask: np.ndarray) -> bool:
        ...
        if not (0.0005 < coverage_ratio < 0.4):
            if self.verbose_logging_enabled:
                logger.debug(f"Region analysis skipped: Signal coverage of {coverage_ratio:.4f} is outside the required range (0.0005-0.4).")
            return False
        ...
        if continuity_ratio < 0.15:
            if self.verbose_logging_enabled:
                logger.debug(f"Region analysis skipped: Signal continuity of {continuity_ratio:.4f} is below the threshold (0.15).")
            return False
    ...
    def _analyze_wave_pattern(self, roi: np.ndarray, region_name: str) -> Optional[WaveAnalysisResult]:
        ...
        if self.verbose_logging_enabled: logger.debug(f"Found {len(y_coords)} total signal pixel coordinates for '{region_name}'.")

        width = color_mask.shape[1]
        
        if len(x_coords) == 0:
            logger.warning(f"No signal pixels found after filtering for '{region_name}'. Creating flat line.")
            ...
            if self.verbose_logging_enabled: logger.debug(f"Created {len(anchor_x)} anchor points for '{region_name}'.")

            if len(anchor_x) < 2:
                logger.warning(f"Signal too sparse ({len(anchor_x)} points found) for '{region_name}'. Defaulting to flat line.")
                ...
                if self.verbose_logging_enabled: logger.debug(f"Successfully interpolated signal across {width} points for '{region_name}'.")
        ...
        if total_energy > 0 and cutoff_indices.size > 0:
        ...
        if self.verbose_logging_enabled: 
            logger.debug(f"Analysis complete for '{region_name}'. HF Energy Ratio: {energy_ratio:.3f}, Class: {'HF' if is_hf else 'LF'}")
    ...
    def _create_visual_log(self, result: WaveAnalysisResult, region_name: str, frame_count: int):
        ...
        try:
            ...
            ax1.plot(result.signal_vector, color='cyan')
            ax1.set_title('Reconstructed Signal', color='white')
            ax1.set_facecolor('#2E2E2E')
            ax1.tick_params(axis='both', colors='white')

            ax2.plot(result.fft_freqs, result.fft_mags, color='magenta')
            ax2.set_title('FFT Magnitude Spectrum', color='white')
            ax2.set_facecolor('#2E2E2E')
            ax2.tick_params(axis='both', colors='white')
            ax2.set_xlabel('Normalized Frequency', color='white')
            ax2.set_ylabel('Magnitude', color='white')
            ax2.axvline(x=self.app_config.fft_cutoff_frequency, color='yellow', linestyle='--', linewidth=1)
            ...
            mask_resized, plot_resized = resize(mask_bgr, h_roi), resize(plot_img, h_roi)

            cls = "HF" if result.is_high_frequency else "LF"
            color = (0, 0, 255) if cls == "HF" else (0, 255, 0)
            text_box = np.zeros((h_roi, 200, 3), dtype=np.uint8)
            cv2.putText(text_box, f"HF Ratio: {result.energy_ratio:.2f}", (10, h_roi//2 - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
            cv2.putText(text_box, f"Result:", (10, h_roi//2 + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
            cv2.putText(text_box, f"{cls}", (30, h_roi//2 + 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
            
            combined = cv2.hconcat([roi_img, mask_resized, plot_resized, text_box])
    ...
    def _create_summary_log(self, results_dict: Dict[str, WaveAnalysisResult], overall_is_hf: bool, frame_count: int):
        ...
        plot_path = f"image_logs/temp_summary_plot_{frame_count}.png"
        try:
            ...
            for i, (name, result) in enumerate(results_dict.items()):
                ...
                title = f"{name} | HF Ratio: {result.energy_ratio:.3f}"
            ...
            fig.suptitle(f"Frame {frame_count} Summary | Avg HF Ratio: {avg_ratio:.3f} | Overall: {classification}", 
    ...
    def _log_results(self, results: Dict, frame_count: int):
        for name, res in results.get('waves', {}).items():
            ...
            logger.info(f"Frame {frame_count}: {name} -> {status} (HF Energy Ratio: {res.energy_ratio:.3f}, HF Energy: {res.high_freq_energy:.2e})")
...
class HSVThresholderWindow(tk.Toplevel):
    def __init__(self, parent, screenshot_cv2, regions, current_config, callback):
        ...
        self.img_label.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        def create_slider(parent, text, var, from_, to):
            ...
            scale = tk.Scale(frame, variable=var, from_=from_, to=to, orient=tk.HORIZONTAL, length=200, command=self._update_mask, showvalue=0)
            scale.pack(side=tk.LEFT, expand=True, fill=tk.X)
            ttk.Button(frame, text="+", width=2, command=lambda: on_change(var.get(), 1)).pack(side=tk.LEFT)
            label = ttk.Label(frame, textvariable=var, width=4)
        ...
        self.hsv_vars['HMin'].set(current_config.hsv_lower[0])
        self.hsv_vars['SMin'].set(current_config.hsv_lower[1])
        self.hsv_vars['VMin'].set(current_config.hsv_lower[2])
        self.hsv_vars['HMax'].set(current_config.hsv_upper[0])
        self.hsv_vars['SMax'].set(current_config.hsv_upper[1])
        self.hsv_vars['VMax'].set(current_config.hsv_upper[2])

        ttk.Button(controls_frame, text="Apply & Close", command=self._apply_and_close).pack(pady=20, side=tk.BOTTOM)
        
        self.grab_set()
        if self.region_selector['values']:
            self.region_selector.current(0)
            self._on_region_select(None)
        
    def _on_region_select(self, _=None):
        region_name = self.region_selector.get()
    ...
    def _update_mask(self, _=None):
        ...
        w, h = img_pil.size
        max_w, max_h = self.img_label.winfo_width(), self.img_label.winfo_height()
    ...
    def _apply_and_close(self):
        hsv_values = {key: var.get() for key, var in self.hsv_vars.items()}
        self.callback(hsv_values)
        self.destroy()
...
class ConfigToolWindow(tk.Toplevel):
    ...
    def _setup_gui(self):
        ...
        screenshot_frame = ttk.LabelFrame(right_panel, text="Capture")
        ...
        params_frame = ttk.LabelFrame(right_panel, text="FFT Analysis Parameters")
    ...
    def _take_screenshot(self):
        self.withdraw(); time.sleep(0.5)
        self.screenshot = cv2.cvtColor(np.array(pyautogui.screenshot()), cv2.COLOR_RGB2BGR)
        self.deiconify(); self.lift(); self.focus_force(); self._display_screenshot()
    ...
    def _on_canvas_click(self, event):
        self.drawing, self.start_x, self.start_y = True, event.x, event.y
    ...
    def _launch_hsv_thresholder(self):
        ...
        if self.screenshot is None:
            ...
            return
        if not wave_regions:
            messagebox.showwarning("Warning", "Please define at least one 'wave' region first.", parent=self)
            return
    ...
    def _apply_new_hsv(self, hsv_values):
        ...
        self.app_config.hsv_lower2 = self.app_config.hsv_lower
        self.app_config.hsv_upper2 = self.app_config.hsv_upper
    ...
    def _save_config(self):
        ...
        try:
            ...
            data['_metadata'] = {
                ...
                'hsv_lower2': self.app_config.hsv_lower2, 
                'hsv_upper2': self.app_config.hsv_upper2, 
    ...
    def _load_config(self):
        ...
        if not path: return
        try:
            temp_monitor = ScreenMonitor(path)
            self.app_config = temp_monitor.app_config
            self._update_ui_from_data()
...
class MonitorControlGUI:
    def __init__(self, root):
        self.root = root; self.root.title("USMA v.0.3.3"); self.root.geometry("650x300")
        ...
        self.image_logging_on = tk.BooleanVar(value=True)
    ...
    def _setup_main_gui(self):
        ...
        self.hf_ratio_var = tk.StringVar(value="Avg HF Ratio: --")
        self.hf_energy_var = tk.StringVar(value="Avg HF Energy: --")
        ...
        debug_frame = ttk.LabelFrame(control_frame, text="Logging")
        debug_frame.pack(side=tk.LEFT, padx=10, pady=5, fill=tk.Y)
        self.verbose_check = ttk.Checkbutton(debug_frame, text="Verbose Log", variable=self.verbose_logging_on)
        self.verbose_check.pack(anchor=tk.W)
        self.img_log_check = ttk.Checkbutton(debug_frame, text="Image Log", variable=self.image_logging_on)
        self.img_log_check.pack(anchor=tk.W)
    ...
    def update_feedback_panel(self, overall_is_hf, avg_ratio, avg_hf_energy):
        """Thread-safe method to update the feedback panel."""
        self.root.after(0, self._update_feedback_ui, overall_is_hf, avg_ratio, avg_hf_energy)

    def _update_feedback_ui(self, overall_is_hf, avg_ratio, avg_hf_energy):
        """The actual UI update logic, run on the main thread."""
        if overall_is_hf:
            self.class_var.set("Overall: HF")
            self.status_light.config(bg="red")
        else:
            self.class_var.set("Overall: LF")
            self.status_light.config(bg="green")
        
        self.hf_ratio_var.set(f"Avg HF Ratio: {avg_ratio:.3f}")
        self.hf_energy_var.set(f"Avg HF Energy: {avg_hf_energy:.2e}")

    def _reset_feedback_ui(self):
        """Resets the feedback panel to its default state."""
        self.class_var.set("Overall: --")
        self.status_light.config(bg="gray")
        self.hf_ratio_var.set("Avg HF Ratio: --")
        self.hf_energy_var.set("Avg HF Energy: --")
    ...
    def _launch_config_tool(self):
        config_window = ConfigToolWindow(self.root); config_window.grab_set()
    ...
    def _toggle_monitoring(self):
        if self.is_monitoring.get():
            ...
            self.verbose_check.config(state=tk.NORMAL)
            self.img_log_check.config(state=tk.NORMAL)
        else:
            ...
            verbose = self.verbose_logging_on.get()
            img_log = self.image_logging_on.get()
            if self.monitor.start(verbose_logging=verbose, image_logging=img_log):
                self.is_monitoring.set(True)
                self.start_stop_button.config(text="Stop Monitoring")
                self.status_label.config(text="Monitoring active...")
                self.load_button.config(state=tk.DISABLED)
                self.edit_button.config(state=tk.DISABLED)
                self.verbose_check.config(state=tk.DISABLED)
                self.img_log_check.config(state=tk.DISABLED)
    ...
    def _toggle_overlay(self):
        ...
        if self.is_overlay_on.get():
            ...
            self.status_label.config(text="Overlay enabled.")
